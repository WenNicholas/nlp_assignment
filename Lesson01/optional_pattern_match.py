...
# print(s)
match = " ".join(
    subsitite("Hi, how do you do? ?*X hello ?*Y".split(), pat_to_dict(pat_match_with_seg('?*X hello ?*Y'.split(),
...
def splitChinese(saying):
    ...
    i = 0
    for v in sayingList:
        if sayingList[i] == '?' and (i + 2) < len(sayingList) and sayingList[i + 1] == '*' and sayingList[
                    i + 2].isalpha():
