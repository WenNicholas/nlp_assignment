...
# -*- coding: UTF-8 -*-
import os
from speech_extraction.data_process.urils_process import deals
from speech_extraction.similar_said.get_word_similar_said import load_said
...
def split_words(sentences):
    sents = [deals(s) for s in sentences]
    print(sents)
    return sents
...
def dependency_parsing(ltp_model_path, sents, postags, said):
    ...
    for index in range(len(sents)):
        ...
        netags = list(netags)
        # print(netags)
        if ('S-Nh' not in netags) and ('S-Ni' not in netags) and ('S-Ns' not in netags):  #人名、机构名、地名  当人名、机构名、地名在该句中则进行依存句法分析
            continue
...
def del_sentences(string):
    path = "E:\pycharmproject\my_project\speech_extraction\similar_said\\"
    said = load_said(path + "similar_said.txt")

    ltp_model_path = 'E:\pycharmproject\my_project\speech_extraction\ltp_data_v3.4.0'
